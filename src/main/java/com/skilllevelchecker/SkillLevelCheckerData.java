...
public class SkillLevelCheckerData
{
    private static final Logger log = LoggerFactory.getLogger(SkillLevelCheckerData.class);
    ...
    public static void loadFromJson(Gson gson) {
        ...
        InputStream infoStream = SkillLevelCheckerData.class.getResourceAsStream("/Items-Information.json");
        if (infoStream == null) {
        ...
        try (InputStreamReader reader = new InputStreamReader(infoStream, StandardCharsets.UTF_8)) {
            Type type = new TypeToken<HashMap<Integer, SkillLevelCheckerEntry>>() {}.getType();
            Map<Integer, SkillLevelCheckerEntry> loaded = gson.fromJson(reader, type);
            for (Map.Entry<Integer, SkillLevelCheckerEntry> e : loaded.entrySet()) {
                int id = e.getKey();
                SkillLevelCheckerEntry entry = e.getValue();
